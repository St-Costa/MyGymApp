...
## Assisted-machine load: subtract from body weight, materialize the same way

An assisted `Exercise` (`loadMode = LoadMode.ASSISTED`, e.g. an assisted pull-up/dip
station) is the mirror image of bodyweight: the number the lifter dials in on the machine
is **assistance**, not load — more assistance selected means *less* real weight lifted.
`Exercise.LoadMode` is mutually exclusive with `isBodyweight` (`ExerciseEditScreen`'s two
switches — "Corpo libero" / "Assistito" — clear each other). Unlike `bwLoadPercent`, there
is no per-exercise default: the assist offset is entered set-by-set during the workout, like
a normal weight, because it changes session to session as the lifter progresses.

The picker on `StrengthExerciseScreen`/`SupersetScreen` stays visible for an assisted
exercise (unlike bodyweight, where it's hidden) and always shows **the number to set on the
machine** — never the net load. On completion, `StrengthExerciseViewModel.buildStrengthSets`
/ `SupersetViewModel.buildUpdatedSession` materialize `weight` =
`materializeAssistedWeight(bodyWeightKg, assistOffsetKg)` = `bodyWeightKg - assistOffsetKg`
(clamped to 0, rounded to 0.5 kg), same `ScaleHistoryRepository.getLatestWeightOnOrBefore`
lookup as bodyweight. The raw offset and base weight are kept on the set as
`assistOffsetKg` + `bwBaseWeightKg` (reused from the bodyweight fields) for audit only.

Because `weight` ends up as the real net load, **tonnage/e1RM/PR math needs no special
casing** — improving on an assisted machine (offset going down) is, by construction, the net
weight going *up*, same direction as any other exercise; see
`ExerciseStatsCalculatorTest`'s "improving on an assisted machine..." test. The only place
that does need special handling is the "previous" pre-fill, which must show the raw assist
number, not the net weight — see the sidecar split below (schema v5).

`materializeAssistedWeight` lives in `TonnageMath.kt` (pure, no Context/IO) and is
unit-tested in `TonnageMathTest`.
...
- **"Previous" is also split by weighting approach (manual load / bodyweight / assisted), not just by slot context.** Sidecar schema v4 introduced the manual/bodyweight split; **schema v5 adds a third slot, `previousSetsAssisted`**, for assisted-machine sets. `previousSets` holds the most recent real *manual-load* session, `previousSetsBodyweight` the most recent real *bodyweight* one (materialized `weight`), `previousSetsAssisted` the most recent real *assisted* one (materialized net `weight`, but the pre-fill reads back `PreviousSet.assistOffsetKg` — the raw machine number — not `weight`). Every consumer that compares today's work against history — the strength/superset grey pre-fill and `ActiveRoutineViewModel`'s change badge — picks via `ContextStats.previousSetsFor(exercise.loadMode)`. Reason for the split: an exercise reconfigured from manual load to bodyweight (Copenhagen adduction went from a `weight: 1.0` placeholder to `bwLoadPercent 75` of ~78 kg) was comparing ~1947 kg against ~32 kg and rendering a **+5005%** change badge, which then line-wrapped the fixed-width badge column into an unreadable smear; the same reasoning applies to a manual↔assisted switch. When the matching-approach slot is empty the exercise reads as "primo dato" (and `TonnageAndRmChange` also clamps any `|pct| ≥ 1000` to `>999%` as a backstop). `pr` / `rmPr` / `hasPriorRealTonnage` deliberately stay all-time across all three approaches — the net-weight materialization makes an assisted set directly comparable to a manual-load one for tonnage/e1RM purposes; only the raw-number pre-fill needs the split.
...
- `stopHrSeriesCapture()` (session end) does a **hard** teardown of everything the session justified: it cancels the reconnect loop, clears the alert, calls `PolarStreamingService.forceStop()` **unconditionally** (a "connectedDevice" FGS has no justification once no session is running — even if the strap stays connected for the HR screen), and calls `api.setAutomaticReconnection(false)`. Turning the SDK auto-reconnect off here is the fix for the recurring "notification stuck after I closed the app and switched the strap off" bug: left on, a later power-off keeps the link in `CONNECTING` and the `deviceDisconnected` teardown never arrives — or the SDK silently reconnects and `deviceConnected()` re-`start()`s the service. `startHrSeriesCapture()` reverses both (auto-reconnect back on, FGS re-`start()`ed) **iff a strap is already connected** as the new session begins.
- **`PolarStreamingService.stop()` cannot rely on `startService`.** When the app is backgrounded and the strap powers off, `context.startService(ACTION_STOP)` throws `BackgroundServiceStartNotAllowedException` and the stop is silently dropped — this is what left the ongoing notification on screen indefinitely. `stop()` now wraps the intent in try/catch and *always* falls through to `forceStop()`, which cancels `NOTIFICATION_ID` + `ALERT_NOTIFICATION_ID` directly via `NotificationManager` and calls `stopService()` (no-op if nothing runs). Every teardown path (`disconnect()`, `shutdown()`, `blePowerStateChanged`, the reconnect-timeout, out-of-session `deviceDisconnected`) routes through `stop()`, so all of them are now background-safe.
- The disconnect alert is cleared on the next `deviceConnected`.
...
`StrengthExerciseViewModel`, `StretchExerciseViewModel`, `SupersetViewModel` and `CardioExerciseViewModel` extend `ui/screen/exercise/ExerciseSessionViewModel`, which owns the shared completion plumbing that was previously copy-pasted (and drifting) across all four: the `clearScope` `SupervisorJob`, `markCompletionAndSave { … }` (runs the completion write on `clearScope`, flips `completionSaved` after), `markSwitched()` (mark done without a write, for "Switch exercise"), and the `onCleared()` template (join the completion job then cancel the scope if finished; otherwise call the subclass's `saveProgressOnExit()`). Subclasses that also run a timer override `onCleared()` to cancel it, then call `super.onCleared()`. (Cardio was the last holdout — its completion used to run on `viewModelScope`, which teardown cancels; its exit path still differs: it persists a resumable open block rather than saving-as-incomplete.)
