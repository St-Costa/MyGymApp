...
## Phase 78 — Supersets up to 3 exercises

Supersets were hard-capped at 2 members: the whole feature — `SupersetViewModel`/`SupersetScreen` (`exercise1`/`exercise2`, `sets1`/`sets2`, `repRangeMin1/2`, `prReps1/2`, `switchEligible1/2`, `switchExercise1/2`, …), the `Superset` nav route (`/{exerciseId1}/{exerciseId2}`), `groupSupersets` (which walked the list two-at-a-time), and the pair containers in the routine editor and active-routine screen — was wired to exactly two.

The persisted model didn't change: `RoutineExercise.supersetWithNext` is already a chain flag, so a routine `A(true) B(true) C(false)` was always *storable*, just not *groupable*. `groupSupersets` now folds a maximal run of consecutive `supersetWithNext` items into one group, capped at `MAX_SUPERSET_SIZE` (3) — a hand-edited YAML with 4+ in a row splits (`[A,B,C] [D]`) and the trailing flag is ignored. The routine editor never creates an over-long chain: `RoutineEditViewModel.toggleSuperset` measures the merged segment before linking and no-ops past 3, and the link button hides in the same case. `ExerciseSegment.SupersetPair(i,j)` → `Superset(indices)`, `ExerciseGroup.Superset(ex1,ex2)` → `Superset(exercises)`, `SupersetPairContainer` → `SupersetContainer` (iterates members with dividers). `SupersetViewModel` is now arity-generic: nav arg `exerciseIds` (comma-separated), `members: List<SupersetMemberUi>`, sets interleaved round-by-round across all members, `buildUpdatedSession` keyed on `exerciseIds.indexOf(...)`, one `switchExerciseAt(memberIndex, id)` + one `switchedMember` flow replacing the numbered pairs. `ExercisePicker` switch key generalised to `pickedExerciseIdSide{1..3}`. New `SupersetGroupingTest` pins the run-folding and the cap; `RoutineParserRoundTripTest` gains a 3-member chain case.

## Phase 79 — Per-exercise stats sidecar: `previous` + PR without scanning history

Opening a strength/superset exercise, or starting a routine, meant reading and parsing **every session file that contains the exercise** — twice (once for the grey "previous" pre-fill via `getSessionsForExercise(id, 30)`, once for the all-time tonnage PR via `getSessionsForExercise(id, Int.MAX_VALUE)`) — and for a superset, ×N members. With ~55 sessions per well-trained exercise that's ~110 Markdown parses per strength screen, ~330 for a 3-member superset, all on the IO thread behind `isLoading`. The x2→x3 superset jump was a visible +50%.

New: `history/_stats/{exerciseId}.yaml` — a materialized view holding, **per slot context** (NORMAL/WARMUP/DAILY), the most recent session's real sets (`previousSets` + `previousSessionDate`), the all-time best `reps*weight` set (`pr`), and `hasPriorRealTonnage`. The exercise ViewModels now do one small YAML read (`workoutRepository.getExerciseStats(id)`) instead of the double history scan; `ActiveRoutineViewModel.exercisesWithPriorTonnage` reads the same flag instead of scanning each exercise's history. Read cost per screen open drops from O(exercise history) to O(1).
