...
- **Exercise progress persistence**: StrengthExercise/StretchExercise/Superset ViewModels save data to disk via `viewModelScope.launch` when "Complete Exercise" is tapped (`completeExercise()`/`completeSuperset()`), then emit `completionSaved = true` via a `MutableStateFlow`. The screen observes this via `LaunchedEffect` and calls `onComplete()` only **after** the save is confirmed on disk. `onCleared()` is reserved for the back-navigation (mid-exercise) case and skips re-saving if already completed. This prevents the race condition where `ActiveRoutineViewModel.markExerciseCompleted` reloads the session from disk before `onCleared()` has finished writing, which would cause `totalTonnage = 0` in the progress chart.
- **onBack vs onComplete**: Exercise screens (`StrengthExerciseScreen`, `StretchExerciseScreen`, `SupersetScreen`) take two separate navigation callbacks: `onBack` (simple pop, no completion signal) and `onComplete` (triggered via `completionSaved` LaunchedEffect, sets `completedExerciseId`/`completedSupersetIds` on `savedStateHandle` then pops). This ensures exercises are marked completed only when the user explicitly taps "Complete Exercise" AND the save is confirmed.
...
- `MainViewModel`: computes gitgraph data from last 28 days in a single loop; produces `gitgraphDays` (DayStatus ×28), `gitgraphTonnageChanges` (Double? ×28), `lastWeekRoutineNames` (String? ×7); color based on **last completed session of each day** vs most recent previous session for that same routine (sorted by `completedAt`); tonnage % change computed at query time (not stored), null if no previous or previous tonnage = 0; `seedDebugData()` for debug: deletes current-week sessions, creates 1 FORZA + 1 STRETCH exercise, 1 routine, previous-week baseline sessions + current-week sessions with varied weights
  - **Common-exercise comparison**: `computeCommonTonnage(s1, s2)` computes tonnage only for exercises present in BOTH sessions (intersection by exerciseId); used for gitgraph status and % — ensures fair comparison when routine composition changes. Falls back to `totalTonnage` if no exercises in common.
...
- Tonnage calculation in `ActiveRoutineViewModel.finalizeSession()`: sum(reps*weight) per exercise and by bodypart, stored in session file; `sessionTonnage` and `sessionTonnageByBodypart` for the progress chart are computed from historical sessions filtered to **only exercises in the current session** (by exerciseId), so the chart is comparable even when the routine composition has changed
...
- `SupersetScreen` / `SupersetViewModel` (`ui/screen/superset/`): loads both exercises + previous FORZA session sets + rep ranges; builds interleaved set list (ex1_set0 → ex2_set0 → …); FORZA pickers pre-populated from previous session values; STRETCH sets centered; `completeSuperset()` saves to disk and emits `completionSaved`
  - **Exercise info cards**: `ExerciseInfoCard` composable shown above the sets for each exercise — primary-bordered Card with exercise name, `MediaPreview` (if link set), and `AutoSaveTextField` for editable notes (auto-saved to `exercise.notes`)
  - **Set grouping**: sets are grouped by superset round — `sets.chunked(2)` wraps each pair (ex1+ex2 for same round) in a primary-bordered Card with no label; inner `SupersetSetItem` cards keep their per-type (orange/blue) border
  - **`isModified` colors**: grey by default; purple only when user explicitly taps +/− (`repsModified`/`weightModified = true`)
- Completion signal: `SupersetScreen.onComplete` (triggered via `completionSaved` LaunchedEffect) sets `completedSupersetIds = "id1,id2"` on `previousBackStackEntry.savedStateHandle`; `AppNavigation` reads it and calls `markExerciseCompleted` for each ID
...
- **ScrollPickerInput scroll bug**: `waitForUpOrCancellation()` returns `null` for BOTH gesture-cancellation (scroll stole the touch) AND genuine timeout (long press). These must be distinguished with a `gestureCancelled` flag set inside the `withTimeoutOrNull` block before the timeout fires. If both cases are treated as long-press, scrolling over a button triggers the repeat loop.
- **completionSaved race condition**: Never call `onComplete()` directly from the button onClick in exercise screens. The save in `onCleared()` is async and `ActiveRoutineViewModel.markExerciseCompleted` reads from disk immediately after navigation. Always use the `completionSaved: StateFlow<Boolean>` + `LaunchedEffect` pattern so navigation happens only after the disk write is confirmed.
